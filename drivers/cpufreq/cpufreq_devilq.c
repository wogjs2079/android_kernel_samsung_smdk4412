/*
 * drivers/cpufreq/cpufreq_devilq.c
 *
 ...
 *
 * Driver values in /sys/devices/system/cpu/cpufreq/devilq
 * 
 */
...
//a hack to make comparisons easier while having different structs in pegasusq and devilq
#define hotplug_history hotplug_devilq_history
#define dvfs_workqueue dvfs_devilq_workqueue

#define devil_VERSION	(2)
#define devil_AUTHOR	"tegrak"

// if you changed some codes for optimization, just write your name here.
#define devil_TUNER "gokhanmoral-robertobsc"
...
struct cpufreq_devil_cpuinfo {
	...
	struct cpufreq_frequency_table devilfreq_table[32];
	unsigned int devilfreq_table_size;
...
static DEFINE_PER_CPU(struct cpufreq_devil_cpuinfo, cpuinfo);
...
static unsigned int get_devilfreq_table_size(struct cpufreq_devil_cpuinfo *pcpu) {
	...
	for (i = 0; (pcpu->freq_table[i].frequency != CPUFREQ_TABLE_END); i++) {
		...
		pcpu->devilfreq_table[size].index = i; //in case we need it later -gm
		pcpu->devilfreq_table[size].frequency = freq;
		size++;
	}
	pcpu->devilfreq_table[size].index = 0;
	pcpu->devilfreq_table[size].frequency = CPUFREQ_TABLE_END;
	return size;
}

static void cpufreq_devil_timer(unsigned long data)
{
	...
	u64 time_in_idle;
	u64 idle_exit_time;
	struct cpufreq_devil_cpuinfo *pcpu =
	...
	if (dbs_tuners_ins.dvfs_debug) {
		printk(KERN_ERR "DevilQ: (cpu %lu) - %s ++\n", data, __func__);
	}
	...
	if (dbs_tuners_ins.ignore_nice) {
		...
		if (dbs_tuners_ins.dvfs_debug) {
			printk(KERN_ERR "DevilQ: [Devil TIMER] cpu %lu, NICE TIME IN IDLE: %u\n",
	...
	if (dbs_tuners_ins.ignore_nice) {
		...
		delta_idle += jiffies_to_usecs(cur_nice_jiffies);
                ...
                if (dbs_tuners_ins.dvfs_debug) {
                        printk(KERN_ERR "DevilQ: [Devil TIMER] cpu %lu, NICE TIME IN RUN: %u\n",
                                        data, jiffies_to_usecs(cur_nice_jiffies));
                }
	}
	...
	if (dbs_tuners_ins.dvfs_debug) {
		printk (KERN_ERR "DevilQ: cpu %lu, load_since_freq_change = %d, load_in_idle = %d\n",
	...
	if ((cpu_load >= inc_cpu_load && !fast_down) || boost_freq) {
		...
		pump_up_step = 2;
			...
			if (pcpu->policy->cur < pcpu->policy->max) {
				ret = cpufreq_frequency_table_target(
					pcpu->policy, pcpu->devilfreq_table,
				...
				new_freq = pcpu->devilfreq_table[index].frequency;
			}
			else
				new_freq = pcpu->policy->max;
		if (dbs_tuners_ins.dvfs_debug) {
			if (pcpu->policy->cur < pcpu->policy->max) {
				printk(KERN_ERR "DevilQ: [PUMP UP] %s, CPU %lu, %d>=%lu, from %d to %d\n",
	   ...
	   unsigned int pump_down_step = 1;
		...
		pump_down_step = 2;
			...
			if (index >= pcpu->devilfreq_table_size) {
				index = pcpu->devilfreq_table_size - 1;
			}

			new_freq = (pcpu->policy->cur > pcpu->policy->min) ? 
				(pcpu->devilfreq_table[index].frequency) :
				(pcpu->policy->min);

		if (dbs_tuners_ins.dvfs_debug) {
			if (pcpu->policy->cur > pcpu->policy->min) {
				printk(KERN_ERR "DevilQ: [PUMP DOWN] %s, CPU %lu, %d<=%lu, from %d to %d\n",
	...
	{
		new_freq = pcpu->policy->cur; //pcpu->devilfreq_table[index].frequency;

		if (dbs_tuners_ins.dvfs_debug) {
			printk (KERN_ERR "DevilQ: [PUMP MAINTAIN] cpu %lu, load = %d, %d\n", data, cpu_load, new_freq);
		}
	...
	if (cpufreq_frequency_table_target(pcpu->policy, pcpu->devilfreq_table,
					   new_freq, CPUFREQ_RELATION_H,
					   &index)) {
		pr_warn_once("DevilQ: timer %d: cpufreq_frequency_table_target error\n",
			     (int) data);
		goto rearm;
	}
	new_freq = pcpu->devilfreq_table[index].frequency;
	...
	if (new_freq < pcpu->target_freq) {
		if (cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_down_time)
		    < down_sample_time) {
			if (dbs_tuners_ins.dvfs_debug) {
				printk (KERN_ERR "DevilQ: [PUMP REARM DOWN]: CPU %lu, (%llu - %llu) < %lu\n",
		...
		if (cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_up_time) <
		    up_sample_time) {
			if (dbs_tuners_ins.dvfs_debug)  {
				printk (KERN_ERR "DevilQ: [PUMP REARM UP]: CPU %lu, (%llu - %llu) < %lu\n",
	...
	if (new_freq < pcpu->target_freq) {
        	if (dbs_tuners_ins.dvfs_debug) {
	            printk (KERN_ERR "DevilQ: [PUMP DOWN NOW] CPU %lu, after %u (run: %llu - last down: %llu), last freq change: %lu\n", 
		...
		if (dbs_tuners_ins.dvfs_debug) {
			printk (KERN_ERR "DevilQ: [PUMP UP NOW] CPU %lu, after %u (run: %llu - last up: %llu), last freq change: %lu\n", 
...
rearm_if_notmax:
	...
	if (dbs_tuners_ins.dvfs_debug) {
		printk (KERN_ERR "DevilQ: cpu %lu, rearm_if_notmax\n", data);
	}
	/*
...
rearm:
	...
	if (dbs_tuners_ins.dvfs_debug) {
		printk (KERN_ERR "DevilQ: cpu %lu, rearm\n", data);
	}
...
exit:
    if (dbs_tuners_ins.dvfs_debug) {
		printk(KERN_ERR "DevilQ: %s (cpu %lu) --\n", __func__, data);
	}
	return;
}

static void cpufreq_devil_idle_start(void)
{
	struct cpufreq_devil_cpuinfo *pcpu =
...
static void cpufreq_devil_idle_end(void)
{
	struct cpufreq_devil_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
...
static int cpufreq_devil_up_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_devil_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
			...
			if (dbs_tuners_ins.dvfs_debug) {
				printk (KERN_ERR "DevilQ: [FREQ UP] CPU %d, Target Up Frequency: %u\n",
								 cpu, max_freq);
			}
...
static void cpufreq_devil_freq_down(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	for_each_cpu(cpu, &tmp_mask) {
		...
		if (dbs_tuners_ins.dvfs_debug) {
			printk (KERN_ERR "DevilQ: [FREQ DOWN] CPU %d, Target Down Frequency: %u\n",
							 cpu, max_freq);
		}
				     ...
				     struct attribute *attr, char *buf)
{
	...
	for (i = 0; i < pcpu->devilfreq_table_size; i++) {
		sprintf(temp, "%u\n", pcpu->devilfreq_table[i].frequency);
		strcat(buf, temp);
	}
...
static void apply_hotplug_lock(void)
{
    ...
    struct cpufreq_devil_cpuinfo *dbs_info;
...
int cpufreq_devilq_cpu_lock(int num_core)
{
	int prev_lock;
...
int cpufreq_devilq_cpu_unlock(int num_core)
{
...
void cpufreq_devilq_min_cpu_lock(unsigned int num_core)
{
	int online, flag;
	struct cpufreq_devil_cpuinfo *dbs_info;
...
void cpufreq_devilq_min_cpu_unlock(void)
{
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret != 1)
		return -EINVAL;
	if (input == 0)
		cpufreq_devilq_min_cpu_unlock();
	else
		cpufreq_devilq_min_cpu_lock(input);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_devilq_cpu_unlock(prev_lock);
	...
	ret = cpufreq_devilq_cpu_lock(input);
	if (ret) {
...
void start_devilq(void);
void stop_devilq(void);

static struct attribute_group devil_attr_group = {
	.attrs = devil_attributes,
    .name = "devilq",
};
...
static void dbs_check_cpu(struct cpufreq_devil_cpuinfo *this_dbs_info)
{
	struct cpufreq_policy *policy;
	int num_hist = hotplug_history->num_hist;
...
{
    struct cpufreq_devil_cpuinfo *dbs_info =
            container_of(work, struct cpufreq_devil_cpuinfo, work.work);
...
static inline void hotplug_timer_init(struct cpufreq_devil_cpuinfo *dbs_info)
{
	...
	case CPUFREQ_GOV_START:
  	...
  	dec_cpu_load_awake = dec_cpu_load;
		...
		for_each_cpu(j, policy->cpus) {
			...
			smp_wmb();
			pcpu->devilfreq_table_size = get_devilfreq_table_size(pcpu);
...
static int cpufreq_devil_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_devil_idle_start();
		break;
	case IDLE_END:
		cpufreq_devil_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_devil_idle_nb = {
	.notifier_call = cpufreq_devil_idle_notifier,
};
...
static struct early_suspend devil_power_suspend = {
	.suspend = devil_early_suspend,
	.resume = devil_late_resume,
...
void start_devilq(void)
{
	...
	up_task = kthread_create(cpufreq_devil_up_task, NULL,
                 "klulzqup");

	sched_setscheduler_nocheck(up_task, SCHED_FIFO, &param);
	get_task_struct(up_task);

	idle_notifier_register(&cpufreq_devil_idle_nb);
	register_early_suspend(&devil_power_suspend);
}
...
void stop_devilq(void)
{
	...
	idle_notifier_unregister(&cpufreq_devil_idle_nb);
	unregister_early_suspend(&devil_power_suspend);
}

static int __init cpufreq_devil_init(void)
{
	...
	if (!hotplug_history) {
		pr_err("%s cannot create devil hotplug history array\n", __func__);
		ret = -ENOMEM;
		goto err_queue;
	}

	dvfs_workqueue = create_workqueue("kdevilq");
	if (!dvfs_workqueue) {
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_devil_timer;
		pcpu->cpu_timer.data = i;
	}

	/* No rescuer thread, bind to CPU queuing the work for possibly
	   warm cache (probably doesn't matter much). */
	down_wq = alloc_workqueue("kdevilq_down", 0, 1);

	if (!down_wq)
		goto err_freeuptask;

	INIT_WORK(&freq_scale_down_work,
		  cpufreq_devil_freq_down);
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_devil
fs_initcall(cpufreq_devil_init);
#else
module_init(cpufreq_devil_init);
#endif
...
module_exit(cpufreq_devil_exit);

MODULE_AUTHOR("Tegrak <luciferanna@gmail.com>");
MODULE_DESCRIPTION("'devilQ' - improved devil governor with hotplug logic");
MODULE_LICENSE("GPL");
